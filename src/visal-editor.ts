 ...
 */
@customElement("visal-editor")
export class VisalEditor extends LitElement {
  ...
  @property() set renderer(value: LineRenderCallback) {
  ...
  buildLines() {
    ...
    for (let i = 0; i < previousNodeList.length; i++) {
      ...
      if (i >= this.lines.length) {
        ...
        let lineContent: Element[] | string = this.lines[i];
        if (this.lineRender) {
  ...
  handleSelectionChange() {
    if (this.shadowRoot) {
      ...
      if (selection && selection.anchorNode && selection.focusNode) {
        ...
        this.dispatchEvent(new CustomEvent("selectchange", { bubbles: false }));
      }
    }
  }
  ...
  removeLine(y1: number) {
    this.lines.splice(y1);
    console.log(this.lines, y1);
    this.buildLines();
  }
  ...
  setSelection(line: number, col: number) {
